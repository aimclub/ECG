...
def get_pqst_peaks(ecg_cleaned, sampling_rate, r_peaks):
    _, peaks = nk.ecg_delineate(
        ecg_cleaned, sampling_rate=sampling_rate, rpeaks=r_peaks, method='peaks', show=False)
...
def get_p_offsets_s_offsets_q_onsets_t_offsets(ecg_cleaned, sampling_rate, r_peaks):
    _, waves_dwt = nk.ecg_delineate(
        ecg_cleaned, sampling_rate=sampling_rate, rpeaks=r_peaks, method='dwt', show=False)
...
def get_ecg_parameters(ecg_cleaned, sampling_rate):
    ...
    ecg_parameters['P_peaks'], ecg_parameters['Q_peaks'], ecg_parameters['S_peaks'], ecg_parameters['T_peaks'] = get_pqst_peaks(
        ecg_cleaned, sampling_rate, ecg_parameters['R_peaks'])

    ecg_parameters['P_offsets'], ecg_parameters['S_offsets'], ecg_parameters['Q_onsets'], ecg_parameters['T_offsets'] = get_p_offsets_s_offsets_q_onsets_t_offsets(
        ecg_cleaned, sampling_rate, ecg_parameters['R_peaks'])
